...
public class FxWeaver {
  ...
  protected <C, V extends Node> FxControllerAndView<C, V> load(Class<C> controllerClass,
    ...
    return Optional.ofNullable(location)
      .map(path -> {
        ...
        return controllerClass.getResource(location);
      })
  ...
  protected String buildFxmlReference(Class<?> c) {
    FxmlView annotation = c.getAnnotation(FxmlView.class);
    if (!annotation.prefix().isEmpty()) {
      return annotation.prefix() + "/" + c.getSimpleName() + ".fxml";
    }
    if (!annotation.value().isEmpty()) {
      return annotation.value();
    }
    return c.getSimpleName() + ".fxml";
  }
